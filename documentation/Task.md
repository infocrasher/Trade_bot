...
- [x] **Fiabilisation LiquidityTracker** (Gate HRLR, pénalités sur exceptions)
- [x] **Scoring OB 5 critères (KB4)** (Création `ob_scorer.py` pour valider fermement les Order Blocks et notation A++)
- [x] **Niveaux Algorithmiques ENIGMA (KB4)** (Création `enigma.py`, +10/-15 pts sur le scoring dans l'orchestrateur et snap du TP1)
- [x] **Malus T-20 Premium (KB4)** (Malus de -20pts ajouté pour les positions Long en zone Premium HTF)
- [x] **State of Delivery (SOD) 5 états (KB4)** (Création de `sod_detector.py` avec sizing dynamique 0%, 50%, 100%)
- [x] **Règle Fateh SOD (P-A4b)** (Distinction HTF/LTF pour le gate ACCUMULATION dans le SOD)
- [x] **Filtres KS4 & KS8 (KB4)** (Blocage sur spread élevé > 3 pips et exclusion des macros NY Morning si CBDR explosif)
